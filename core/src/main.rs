...
generate_list(N, cons(N, T)) :- >(N, 1), is(M, -(N, 1)), generate_list(M, T).
abs(0, 0).
abs(N, N) :- >(N, 0).
abs(N, M) :- <(N, 0), is(M, *(N, -1)).
n_queens(N, Qs) :- generate_list(N, Qs1), perm(Qs1, Qs), safe_queens(Qs).
safe_queens(nil).
safe_queens(cons(Q, Qs)) :- safe_queens(Qs, Q, 1), safe_queens(Qs).
safe_queens(nil, Y, X).
safe_queens(cons(Q, Qs), Q0, D0) :- =\=(Q0, Q), is(Diff, -(Q0, Q)), abs(Diff, AbsDiff), =\=(AbsDiff, D0), is(D1, +(D0, 1)), safe_queens(Qs, Q0, D1).
"#;
